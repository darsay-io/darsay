...
| `precision` | The release precision label — `BF16`, `FP8`, `MXFP4`, `AWQ INT4`, `Q4_K_M` — from `config.json`'s `quantization_config` (wherever a multimodal config nests it), else the dominant tensor dtype, else a GGUF-only payload's file name ([Quantization §2](QUANTIZATION.md#2-precision-and-size-scope)). Precision does not establish original-release status. |
...
| `references` | What the tree names that lives elsewhere ([Code bundles § 7](CODE.md#7-what-the-tree-references)). `read_from: "payload"`; `scan` (`files_scanned`, `bytes_scanned`, `skipped` by reason, `partial` when a read budget bound); `items[]`, each `{ref, kind, tier, declared_by, found_in[], occurrences, resolved, revision | digest}` — `ref` a canonical address (`huggingface:owner/name`, `huggingface:datasets/owner/name`, `oci:image:tag`, `github:owner/repo`), `kind` ∈ `model` / `dataset` / `image` / `code`, `tier` ∈ `declared` / `evidence` / `mentioned`, `declared_by` ∈ `env_template` / `compose` / `dockerfile` / `spaces_card` / `shell_default` / `literal` / `url`, `found_in` up to 10 `path:line` with the strongest first, `resolved` true / false / null (null: not asked — a mention, an image, or the lookup failed), `revision` always null (the tree names no revision; the vault's bundle is the pinned instance), `digest` null until an image provider records one; `resolved_at`, `query_limit` (20); `primary_model` `{ref, rule, candidates[], reason}` — the one model at evidence or better that resolves, else null with the reason. Strings found in files, never a claim about what the code loads at run time. |
...
([Concepts → lineage](CONCEPTS.md#lineage)).

| Field | Meaning |
|---|---|
| `parents` | Edges to the works this one was made from, each `{source, relation, declared_by}`. `source` is a canonical ref (`huggingface:Qwen/Qwen3.8-27B`, `huggingface:datasets/owner/name`). `relation` is the label upstream gave the edge — `finetune`, `adapter`, `merge`, `quantized` (the Hub's model-tree relations), `trained_on` (from the card's `datasets`), `derived` (a dataset's `source_datasets`), `fork` (a GitHub repository's upstream), `references` (a code bundle's primary model: the one model its code names that resolves upstream, [Code bundles § 7](CODE.md#7-what-the-tree-references)) — or null when a parent is declared without a label. `declared_by` is where the declaration was read: `card`, `tag`, `api` (the fork edge), or for `references` the provenance that named it — `env_template`, `compose`, `dockerfile`, `spaces_card`, `shell_default`, `literal`, `url`. Null when nothing is declared. The Hub has no relation for alignment edits (abliteration); those surface in `identity.variants` (read from the name) and in `curation.md`. |
