...
def _dead_doctor_lease(bundle: Path, run_id: str) -> None:
...
def test_prepared_only_journal_blocks_new_runs_until_strict_undo(vault, capsys):
    ...
    _dead_doctor_lease(bundle, run_id)
...
def test_prepared_before_commit_reclaims_dead_lease_without_inverse_write(
    vault, capsys
):
    from darsay.doctor import _append_action, _new_run
    ...
    _append_action(
        run,
        {
            "schema_version": 1,
            "event": "mutation_prepared",
            "action_id": "interrupted-before-commit",
            "action": "WriteFile",
            "op": "WriteFile",
            "run_id": run_id,
            "fixer_id": "bundle.readme.regenerate",
            ...
            "before_mode": before_info.st_mode & 0o7777,
            "before_mtime_ns": before_info.st_mtime_ns,
            "after_sha256": hashlib.sha256(b"uncommitted generated bytes").hexdigest(),
            "started_at_ns": 1,
            "finished_at_ns": None,
            "ok": None,
            "rolled_back": False,
    ...
    assert report["target_actions"] == 0
    assert target.read_bytes() == before
    assert target.stat().st_mtime_ns == before_info.st_mtime_ns
    assert not (bundle / "transfer.lock").exists()
    events = [
        json.loads(line) for line in (run / "actions.jsonl").read_text().splitlines()
    ]
    assert events[-1]["event"] == "recovery_finished"
    assert events[-1]["action_ids"] == ["interrupted-before-commit"]
