...
def test_save_writes_the_current_schema_version(tmp_path):
    ...
    raw = _catalog([_entry("huggingface:acme/toy")], catalog_schema_version="3.2.0")
    path.write_text(json.dumps(raw), encoding="utf-8")
    loaded = load_catalog(path)
    assert loaded["catalog_schema_version"] == "3.2.0"
    save_catalog(path, loaded)
    again = json.loads(path.read_text(encoding="utf-8"))
    assert again["catalog_schema_version"] == CATALOG_SCHEMA_VERSION == "3.2.0"
